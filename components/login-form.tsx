...
// Asumiendo que tienes estos SVGs o componentes para los botones de OAuth
 ...
 return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          <label htmlFor="password" className="text-sm font-medium text-foreground">{t('passwordLabel')}</label>
          <div className="relative">
            <Input id="password" name="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" required value={formData.password} onChange={handleChange} className="pr-10" />
...
<div className="flex flex-wrap gap-2 flex-row items-center justify-center">
        <Button variant="outline" className="bg-transparent"><GoogleIcon />Google</Button>
        <Button variant="outline" className="bg-transparent"><GitHubIcon />GitHub</Button>
      </div>

      <p className="text-center text-sm text-muted-foreground">
        {t('noAccount')}{' '}
        <Link href="/register" className="underline text-primary">{t('signUpLink')}</Link>
      </p>
    </motion.div>
  );
}
